...
function authorsToStrings(paper) {
    return (paper.authors ?? []).map((a) => a.name).filter((name) => Boolean(name));
}
export function paperToItem(paper) {
...
export async function handleDiscoverSearch(client, query, limit, tableMode) {
...
export async function handleDiscoverLookup(client, opts, tableMode) {
    const result = await client.semanticScholarLookup(opts);
    format(result, tableMode);
}
export async function handleDiscoverList(client, kind, paperId, limit, tableMode) {
    const result = await client.semanticScholarPaperList(kind, paperId, limit);
    format(result, tableMode, ['paper_id', 'title', 'year', 'venue', 'citation_count', 'open_access_pdf_url']);
}
export async function handleDiscoverAdd(client, vaultId, filePath, idempotencyKey, tableMode) {
    const papers = JSON.parse(readFileSync(filePath, 'utf8'));
...
function addPaperListCommand(discover, name, description) {
    discover
        .command(name)
