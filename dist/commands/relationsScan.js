...
export function findMatchingItem(paper, items, excludeId) {
...
function relationExists(relations, publicationId, relatedPublicationId) {
 ...
 */
export async function scanItemRelations(item, client, vaultId, allItems, existingRelations, opts) {
    if (!item.doi)
        return { outcome: 'skipped_no_doi', suggested: 0, created: 0, skippedDuplicate: 0 };
    const lookup = await client.semanticScholarLookup({ doi: item.doi });
    const paperId = lookup.data.paper_id;
    if (!paperId)
        return { outcome: 'lookup_failed', suggested: 0, created: 0, skippedDuplicate: 0 };
...
export function registerRelationsScan(relations) {
    relations
        .command('scan')
